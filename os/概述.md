...
idle 0 填满cpu使用时间
**os是中断驱动的**
- io设备通知cpu(有int引脚或控制芯片可以并联$\rightarrow$接受io设备的电信号, 发生变化后读取中断寄存器$\rightarrow$硬件提供机制,查中断向量表(一组地址,硬件定义长度),执行对应地址上的代码)
- 初始化一个定时器: 每隔一段时间产生一个中断

<!--stackedit_data:
eyJoaXN0b3J5IjpbMTc3NTQ2ODc2MywxMjAzMDE4OTU4LDE1Mz
